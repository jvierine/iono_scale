...
for i in range(len(d)):
    ...
    for j in range(32):
        ...
        if pr[j,2] > 0.9 or pr[j,3] > 0.9:
            plt.axhline(f2_pr[j,0]*100.0,color="red")
            plt.axvline(f2_pr[j,1],color="red")
        if pr[j,0] > 0.9:
            plt.axhline(es_pr[j,0]*100.0,color="white")
            plt.axvline(es_pr[j,1],color="white")
        if pr[j,1] > 0.9:
            plt.axhline(e_pr[j,0]*100.0,color="blue")
            plt.axvline(e_pr[j,1],color="blue")
        if pr[j,2] > 0.9:
            plt.axhline(f1_pr[j,0]*100.0,color="green")
            plt.axvline(f1_pr[j,1],color="green")
